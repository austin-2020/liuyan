...
<body style="background-color: #f7da55">
  ...
  <?php
	$con=@mysqli_connect("127.0.0.1","root","1234","zzh");
	if (!$con) {
	...
	$query_sql="select * from message  order by user desc limit $offset,$pagesize";
	...
	while ($res=mysqli_fetch_array($result)) {
		...
		//分页代码  
        //计算留言总数  
        $count_result = mysqli_query($con,"select count(*) as count from message ");
